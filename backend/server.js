...
app.use(
  ...
  rateLimit({
    ...
    max: 30,
    keyGenerator: (req) =>
      req.authUser?.id || req.socket?.remoteAddress || "unknown",
    message: {
      error: "Bir daqiqada ko'pi bilan 30 ta so'rov yuborishingiz mumkin.",
    },
  }),
);
...
app.use(
  ...
  rateLimit({
    ...
    message: {
      error:
        "Ro'yxatdan o'tish cheklandi. 1 soatdan keyin qayta urinib ko'ring.",
    },
  }),
);
...
app.use((err, _req, res, _next) => {
  if (err.code === "LIMIT_FILE_SIZE") {
    return res
      .status(400)
      .json({ error: "Rasm hajmi 5MB dan oshmasligi kerak" });
  }
...
async function fixUserNullFields() {
  ...
  if (r1.modifiedCount || r2.modifiedCount) {
    console.log(
      `✅ Foydalanuvchi maydonlari tuzatildi (googleId: ${r1.modifiedCount}, telegramId: ${r2.modifiedCount})`,
    );
  }
}
