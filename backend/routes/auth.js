  ...
  async (req, res) => {
    try {
      ...
      if (telegramUsername) {
        ...
        console.log(
          `💾 Saved pendingTelegramUsername for user ${user.username}: ${normalizedUsername}`,
        );
      }
      await user.save();
...
router.get(
  ...
  async (req, res) => {
    try {
      const user = await User.findById(req.authUser.id)
        .select(
          "telegramId telegramVerified telegramUsername pendingTelegramUsername",
        )
        .lean();
      if (!user) return res.status(404).json({ error: "Topilmadi" });
      return res.json({
        ...
        pendingTelegramUsername: user.pendingTelegramUsername,
      });
    } catch {
      return res.status(500).json({ error: "Server xatosi" });
    }
