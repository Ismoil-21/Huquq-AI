...
router.post("/register", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Register OTP email xatosi:", mailErr.message);
      // Email yuborilmasa user ni o'chirib qaytamiz
      await User.deleteOne({ _id: user._id });
      return res.status(500).json({ error: "Tasdiqlash kodi yuborilmadi. Email manzilingizni tekshiring yoki keyinroq urinib ko'ring." });
    }
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    const botUsernameV = process.env.TELEGRAM_BOT_USERNAME || "mening_huquqlarim_bot";
